...
export async function runLighthouseMedianAtPort(
  ...
  for (let i = 0; i < 3; i++) {
    // Launch a fresh Chrome for every run — reusing the same Chrome instance across
    // runs causes stale 'lh:runner:gather' performance marks that crash Lighthouse.
    ...
    try {
      ...
      try { chrome.kill(); } catch { /* ignore kill errors */ }
    }
  }
